...
/* == Conectando a REDE == */
#define NAME_WIFI "Vini"
#define PASSWORD_WIFI "vini2131"
#define MAXIMUM_RETRY 5
static const char *TAAG = "@vynizinho";
                                ...
                                int32_t event_id, void* event_data)
{
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        ...
        if (s_retry_nume < MAXIMUM_RETRY) {
            esp_wifi_connect();
            s_retry_nume++;
            ESP_LOGI(TAAG, "retry to connect to the AP");
        } else {
            xEventGroupSetBits(s_wifi_event_group, WIFI_FAIL_BIT);
        }
        ESP_LOGI(TAAG,"connect to the AP fail");
    } else if (event_base == IP_EVENT && event_id == IP_EVENT_STA_GOT_IP) {
        ip_event_got_ip_t* event = (ip_event_got_ip_t*) event_data;
        ESP_LOGI(TAAG, "static ip:" IPSTR, IP2STR(&event->ip_info.ip));
        s_retry_nume = 0;
...
void connect_wifi(void)
{
    ...
    ESP_LOGI(TAAG, "Função wifi finalizada!!!");
    ...
    {
        ESP_LOGI(TAAG, "Conectado ao SSID:%s password:%s",
                 NAME_WIFI, PASSWORD_WIFI);
    }
    else if (bits & WIFI_FAIL_BIT)
    {
        ESP_LOGI(TAAG, "A conexão ao SSID:%s, password:%s falhou!!!",
                 NAME_WIFI, PASSWORD_WIFI);
    }
    else
    {
        ESP_LOGE(TAAG, "Evento inesperado!!!");
    }
